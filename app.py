...
try:
    ...
    st.error("HOME_DATA.csv 파일을 불러올 수 없습니다. GitHub 저장소에 파일이 올바르게 업로드되었는지 확인해주세요.")
    st.stop()
...
if pred_mid < bep_sales:
    st.error(f"🚨 **[손실 위험]** 예상 주문액({int(pred_mid):,}원)이 BEP({int(bep_sales):,}원)에 미치지 못해 약 **{abs(int(est_profit)):,}원**의 결손이 예상됩니다. PPL 단가나 편성 조건을 재검토하세요.")
else:
...
with st.expander("🔍 분석에 사용된 과거 유사 방송 실적 보기"):
    ...
    st.dataframe(ref_df[['일자', '상품명', '홈쇼핑 채널', '시작시간', '주문금액', '달성율', '이익금', 'PPL PGM']])
